resource "aws_iam_role"  "ecr_readOnly_role" {
  name_prefix = "${format("ecr_readOnly_role_%s_", terraform.workspace)}"
...
resource "aws_iam_instance_profile" "ecr_readOnly_profile" {
  name_prefix = "${format("ecr_readOnly_profile_%s", terraform.workspace)}"
  role = "${aws_iam_role.ecr_readOnly_role.name}"
}

resource "aws_iam_role_policy" "ecr_readOnly_policy" {
  name_prefix = "${format("ecr_readOnly_policy_%s", terraform.workspace)}"
